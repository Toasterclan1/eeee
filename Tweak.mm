...
static void giveSelfMoney(unsigned int amount) {
    ...
    if (!giveSelfMoneyMethod) { NSLog(@"[ACMod] giveSelfMoney: method not found"); return; }
    ...
    if (exc) NSLog(@"[ACMod] Exception while giving money");
...
static void giveAllPlayersMoney(int amount) {
    ...
    if (gameManagerAddPlayerMoneyMethod) {
        ...
        if (!exc) { NSLog(@"[ACMod] GameManager.AddPlayerMoney OK"); return; }
    }
    NSLog(@"[ACMod] Falling back to giveSelfMoney");
...
// =============================================================================
//  ⑦ CATEGORY: op — IL2CPP / RPC helpers  (declared BEFORE SpawnerPanel)
// =============================================================================
...
+ (void)op_giveSelfMoney:(unsigned int)amount {
    ...
    if (!op_giveSelfMoneyMethod) { NSLog(@"[op] AddPlayerMoney not found"); return; }
    ...
    if (exc) NSLog(@"[op] Exception giving money");
    else     NSLog(@"[op] Gave %u to local player", amount);
}

+ (void)op_giveAllPlayersMoney:(int)amount {
    if (op_rpcAddPlayerMoneyToAllMethod && op_findObjectOfTypeMethod &&
        op_il2cpp_class_get_type && op_il2cpp_type_get_object) {
...
+ (void)op_spawnItem:(NSString *)itemName quantity:(int)quantity x:(float)x y:(float)y z:(float)z {
    if (!op_spawnItemMethod || !op_il2cpp_string_new) {
        // Fallback to hook-based spawn
        executeSpawn(itemName, x, y, z, quantity);
        return;
    }
...
// =============================================================================
//  ④ UI — overlay panel (ITEMS / PLAYERS / CHEATS tabs)
// =============================================================================
...
- (void)buildUI {
    ...
    UIBezierPath* bp = [UIBezierPath bezierPathWithRoundedRect:tb.bounds
    ...
    ttl.attributedText = as;
    [tb addSubview:ttl];

    UIButton* xb = [UIButton buttonWithType:UIButtonTypeSystem];
    xb.frame = CGRectMake(W-40, 8, 28, 24);
    [xb setTitle:@"✕" forState:UIControlStateNormal];
    xb.titleLabel.font = [UIFont systemFontOfSize:14];
    [xb setTitleColor:UIColor.blackColor forState:UIControlStateNormal];
    [xb addTarget:self action:@selector(closePanel) forControlEvents:UIControlEventTouchUpInside];
    [tb addSubview:xb];
    ...
    [self addSubview:ic]; self.itemsContainer = ic;
    CGFloat y = 4;
    ...
    tv.dataSource = self; tv.delegate = self; tv.rowHeight = 28;
    ...
    tv.layer.cornerRadius = 6; tv.clipsToBounds = YES; tv.tag = 1;
    [ic addSubview:tv]; self.itemTable = tv; y += 121;

    UILabel* qLbl = [[UILabel alloc] initWithFrame:CGRectMake(pad, y+4, 70, 24)];
    qLbl.text = @"QUANTITY"; qLbl.font = [UIFont boldSystemFontOfSize:10]; qLbl.textColor = UIColor.blackColor;
    [ic addSubview:qLbl];
    UILabel* qVal = [[UILabel alloc] initWithFrame:CGRectMake(78, y, 40, 32)];
    qVal.text = [NSString stringWithFormat:@"%ld", (long)spawnQuantity];
    qVal.font = [UIFont boldSystemFontOfSize:18]; qVal.textColor = UIColor.blackColor;
    ...
    NSArray* axLabels = @[@"X", @"Y", @"Z"];
    float cv[3] = {customSpawnX, customSpawnY, customSpawnZ};
    NSArray* cFields = @[
    ...
    for (int ci = 0; ci < 3; ci++) {
        ...
        al.text = axLabels[ci]; al.font = [UIFont boldSystemFontOfSize:10];
        al.textColor = UIColor.blackColor; al.textAlignment = NSTextAlignmentCenter;
        [ic addSubview:al];
        UITextField* af = cFields[ci];
        af.text = [NSString stringWithFormat:@"%.2f", cv[ci]];
        af.frame = CGRectMake(fx, y+16, fw, 30);
        [ic addSubview:af];
    }
    y += 54;

    UIButton* spawnBtn = [UIButton buttonWithType:UIButtonTypeSystem];
    spawnBtn.frame = CGRectMake(pad, y, W-pad*2, 36);
    ...
    stl.font = [UIFont monospacedSystemFontOfSize:10 weight:UIFontWeightRegular];
    stl.textColor = UIColor.blackColor; stl.textAlignment = NSTextAlignmentCenter; stl.numberOfLines = 2;
    [ic addSubview:stl]; self.statusLabel = stl;

    // ── PLAYERS CONTAINER ──────────────────────────────────────────────────
    UIView* pc = [[UIView alloc] initWithFrame:CGRectMake(0, cY, W, cH)];
    pc.backgroundColor = UIColor.clearColor; pc.hidden = YES;
    [self addSubview:pc]; self.playersContainer = pc;
    CGFloat py2 = 4;

    UILabel* ph = [[UILabel alloc] initWithFrame:CGRectMake(pad, py2+4, W-pad*2-70, 20)];
    ph.text = @"👥 LOBBY PLAYERS"; ph.font = [UIFont boldSystemFontOfSize:10]; ph.textColor = UIColor.blackColor;
    [pc addSubview:ph];

    UIButton* rfBtn = [UIButton buttonWithType:UIButtonTypeSystem];
    rfBtn.frame = CGRectMake(W-pad-64, py2, 64, 28);
    [rfBtn setTitle:@"⟳ REFRESH" forState:UIControlStateNormal];
    rfBtn.titleLabel.font = [UIFont boldSystemFontOfSize:9];
    rfBtn.backgroundColor = [UIColor colorWithWhite:0.82 alpha:1];
    rfBtn.layer.cornerRadius = 5; rfBtn.layer.borderWidth = 1;
    rfBtn.layer.borderColor = [UIColor colorWithWhite:0.65 alpha:1].CGColor;
    ...
    ptv.dataSource = self; ptv.delegate = self; ptv.rowHeight = 44;
    ptv.backgroundColor = UIColor.whiteColor;
    ptv.separatorColor  = [UIColor colorWithWhite:0.85 alpha:1];
    ptv.layer.borderWidth = 1;
    ptv.layer.borderColor = [UIColor colorWithWhite:0.7 alpha:1].CGColor;
    ptv.layer.cornerRadius = 6; ptv.clipsToBounds = YES; ptv.tag = 2;
    [pc addSubview:ptv]; self.playerTable = ptv; py2 += 181;

    UIButton* satBtn = [UIButton buttonWithType:UIButtonTypeSystem];
    satBtn.frame = CGRectMake(pad, py2, W-pad*2, 36);
    [satBtn setTitle:@"SPAWN AT PLAYER" forState:UIControlStateNormal];
    satBtn.titleLabel.font = [UIFont boldSystemFontOfSize:14];
    satBtn.backgroundColor = UIColor.whiteColor;
    satBtn.layer.borderWidth = 2;
    satBtn.layer.borderColor = [UIColor colorWithWhite:0.65 alpha:1].CGColor;
    satBtn.layer.cornerRadius = 6;
    ...
    cc.backgroundColor = UIColor.clearColor; cc.hidden = YES;
    [self addSubview:cc]; self.cheatsContainer = cc;

    __block CGFloat cy2 = 4;
    CGFloat bh = 36, gap = 8;
    ...
    void (^addBtn)(NSString*, NSString*) = ^(NSString* lbl, NSString* sel) {
        ...
        b.layer.borderColor = [UIColor colorWithWhite:0.65 alpha:1].CGColor;
    ...
    mh.text = @"💰 MONEY"; mh.font = [UIFont boldSystemFontOfSize:10]; mh.textColor = UIColor.blackColor;
    [cc addSubview:mh]; cy2 += 20;
    ...
    ch.text = @"⚡ CHEATS"; ch.font = [UIFont boldSystemFontOfSize:10]; ch.textColor = UIColor.blackColor;
    [cc addSubview:ch]; cy2 += 20;
    ...
    comh.text = @"🌐 COMMUNITY"; comh.font = [UIFont boldSystemFontOfSize:10]; comh.textColor = UIColor.blackColor;
    [cc addSubview:comh]; cy2 += 20;
...
- (void)spawnTapped:(UIButton*)btn {
    ...
    if (!self.selectedItem) {
        [self showStatus:@"!! no item selected !!" color:[UIColor systemRedColor]]; return;
    }
...
- (void)giveInfAmmo {
    int64_t player = getLocalPlayer();
    if (!player) { [self showStatus:@"Could not find local player" color:UIColor.blackColor]; return; }
...
static void dylibMain() {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0*NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        presetLocationNames = @[@"Custom",@"Spawn",@"Forest",@"Mountain",@"Beach",@"Cave",
                                @"Village",@"Ruins",@"River",@"Swamp",@"Desert",@"Tundra",@"Sky"];
        for (int i = 0; i < 13; i++) presetLocationCoords[i] = (Vec3){0.f, 3.f, 0.f};
